      ...
      if (ivSideGrid[a_idir] == ivSideDom[a_idir])
        {
	      ...
	      for (int k=0;k<a_faceData.nComp();k++)
		{
		  Real extrapValue = extrapFaceGradToBoundary(bndryFace,sit(),a_idir,a_ebGraph,a_faceData,k);
		  a_faceData(bndryFace,k) = extrapValue;
		}
            }
        }
    }
...
Real extrapFaceGradToBoundary(const FaceIndex&      a_bndryFace,
				  const Side::LoHiSide& a_side,
				  const int&            a_idir,
				  const EBGraph&        a_ebGraph,
				  const EBFaceFAB&      a_faceData,
...
/*****/
void
extrapolateCellToEB(EBFaceFAB &             a_faceData,
                                 const EBGraph &         a_ebGraph,
                                 const Box &             a_grid,
                                 const int &             a_idir,
                                 const ProblemDomain &   a_domain,
