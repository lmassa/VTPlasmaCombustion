...
densityIndexC() const
{
  return CRHO; //luca added not sure CRHO==QRHO
}
/******/
int
EBPatchPolytropic::
...
void EBPatchPolytropic::expressions(HDF5HeaderData& a_expressions)
{
  ...
  a_expressions.m_string["scalar Mach"] = "sqrt(2*kinetic_energy)/soundspeed";
                                  ...
                                  const Box&  a_box)
{
  ...
  for (VoFIterator vofit(ivsIrreg, m_ebisBox.getEBGraph());
      vofit.ok(); ++vofit)
    {
      const VolIndex& vof = vofit();
      //lucacancel 20.0
      a_consState(vof, CRHO) = Min(Max(a_consState(vof, CRHO), smallr),20.0);
      a_consState(vof, CENG) = Min(Max(a_consState(vof, CENG), small ),20.0);
      for (int idir = 0; idir < SpaceDim; idir++)
        {
      //lucacancel momentum flooring
          a_consState(vof, CMOMX + idir) = Min(Max(a_consState(vof, CMOMX + idir), -50.0),50.0);
        }
