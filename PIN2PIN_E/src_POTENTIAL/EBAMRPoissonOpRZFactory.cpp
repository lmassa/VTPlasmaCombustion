...
//-----------------------------------------------------------------------
void
coarsenStuffRZ(LevelData<EBCellFAB>               & a_acoefCoar,
...
//-----------------------------------------------------------------------
EBAMRPoissonOpRZFactory::~EBAMRPoissonOpRZFactory()
{
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
EBAMRPoissonOpRZFactory::
EBAMRPoissonOpRZFactory(const Vector<EBLevelGrid>&                                     a_eblgs,
    ...
    if ((ilev==0) || (m_refRatio[ilev] > 2))
    {
      ...
      while (hasCoarser)
      {
        ...
        if (hasCoarser)
        {
          ...
          coarsenStuffRZ(*acoefCoar, *bcoefCoar, *bcoefIrregCoar, eblgFine, eblgCoar,
        ...
        bool                 a_homoOnly)
{
  ...
  //
  EBAMRPoissonOpRZ* newOp = NULL;
  if (acoef0 == NULL)
  {
    // Time-independent A coefficient.
    newOp = new EBAMRPoissonOpRZ(EBLevelGrid(), eblgMGLevel, EBLevelGrid(), eblgCoarMG, quadCFI,
...
AMRnewOp(const ProblemDomain& a_domainFine)
{
  ...
  {
    // Time-independent A coefficient.
    newOp = new EBAMRPoissonOpRZ(eblgFine, eblgMGLevel, eblgCoar, eblgCoarMG, m_quadCFI[ref],
...
void
EBAMRPoissonOpRZFactory::
AMRreclaim(EBAMRPoissonOpRZ* a_reclaim)
{
  delete a_reclaim;
}
//-----------------------------------------------------------------------
