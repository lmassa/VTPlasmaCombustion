...
#include "EBAMRPoissonOpRZ.H"
#include "EBQuadCFInterp.H"

#include "EBAMRPoissonOpRZF_F.H"
#include "InterpF_F.H"
...
//IntVect EBAMRPoissonOpRZ::s_ivDebug = IntVect(D_DECL(111, 124, 3));
bool EBAMRPoissonOpRZ::s_turnOffBCs = false; //REALLY needs to default to false
bool EBAMRPoissonOpRZ::s_forceNoEBCF = false; //REALLY needs to default to false

//-----------------------------------------------------------------------
EBAMRPoissonOpRZ::
EBAMRPoissonOpRZ(const EBLevelGrid &                                  a_eblgFine,
  ...
  : LevelTGAHelmOp<LevelData<EBCellFAB>, EBFluxFAB>(false), // is time-independent
    ...
    m_colors()
{
  CH_TIME("EBAMRPoissonOpRZ::PoissonOpRZ");
  int ncomp = 1;
...
{
  const EBAMRPoissonOpRZ& op =
    dynamic_cast<const EBAMRPoissonOpRZ&>(a_operator);
...
{
  CH_TIME("EBAMRPoissonOpRZ::getDivFStencil");
               ...
               const DataIndex& a_dit)
{
  ...
  CH_TIME("EBAMRPoissonOpRZ::getFluxStencil");
...
getFaceCenteredFluxStencil(VoFStencil&      a_fluxStencil,
                           ...
                           const DataIndex& a_dit)
{
  CH_TIME("EBAMRPoissonOpRZ::getFaceCenteredFluxStencil");
...
setAlphaAndBeta(const Real& a_alpha,
                const Real& a_beta)
{
  CH_TIME("EBAMRPoissonOpRZ::setAlphaAndBeta");
  m_alpha = a_alpha;
  m_beta  = a_beta;
...
{
  CH_TIME("EBAMRPoissonOpRZ::kappaScale");
              ...
              bool a_kappaWeighted)
{
  ...
  CH_TIME("EBAMRPoissonOpRZ::diagonalScale");
  //  dumpLevelPoint(a_rhs, string("EBAMRPoissonOpRZ: diagonalScale: phi coming in = "));
  if (a_kappaWeighted)
    EBLevelDataOps::kappaWeight(a_rhs);

  //  dumpLevelPoint(a_rhs, string("EBAMRPoissonOpRZ: diagonalScale: kappa*phi = "));
  ...
  //  dumpLevelPoint(a_rhs, string("EBAMRPoissonOpRZ: diagonalScale: acoef*kappa*phi = "));
...
divideByIdentityCoef(LevelData<EBCellFAB> & a_rhs)
{
  ...
  CH_TIME("EBAMRPoissonOpRZ::divideByIdentityCoef");
...
defineStencils()
{
  CH_TIME("EBAMRPoissonOpRZ::defineStencils");
...
{
  CH_TIME("EBAMRPoissonOpRZ::residual");
...
preCond(LevelData<EBCellFAB>&       a_lhs,
        const LevelData<EBCellFAB>& a_rhs)
{
  CH_TIME("EBAMRPoissonOpRZ::preCond");
...
incrOpRegularAllDirs(Box * a_loBox,
                     ...
                     const DataIndex& a_dit)
{
  CH_TIME("EBAMRPoissonOpRZ::incrOpRegularAllDirs");
...
applyDomainFlux(Box * a_loBox,
                ...
                const DataIndex& a_dit)
{
  CH_TIME("EBAMRPoissonOpRZ::applyDomainFlux");
  CH_assert(m_domainBC != NULL);
...
//    }
  ...
  //  dumpFABPoint(a_lhs, a_datInd, string("EBAMRPoissonOpRZ::applyopirr before apply lhs="));
  ...
  //  dumpFABPoint(a_lhs, a_datInd, string("EBAMRPoissonOpRZ::applyopirr after  apply lhs="));
...
void  EBAMRPoissonOpRZ::
setEBBC(const RefCountedPtr<ConductivityBaseEBBC>&      a_ebBC)
{
  m_ebBC     = a_ebBC;
}

void EBAMRPoissonOpRZ::
setEBBCSource(const Vector<LevelData<BaseIVFAB<Real> >* > a_rhos, const Vector<ProblemDomain>& a_domains)
{
  bool found = false;
  int whichlev;
  for (int ilev = 0; ilev < a_rhos.size(); ilev++)  
    {
      if(a_domains[ilev] == m_eblg.getDomain())
        {
          found = true;
          whichlev = ilev ;
        }
  ...
  if (!found)
    {
      MayDay::Error("EBAMRPoissonOpRZ::setEBBCSource cant find level");
    }
  m_ebBC->setEBBCSource(*a_rhos[whichlev]);
}

void EBAMRPoissonOpRZ::
zeroEBBCSource()
{ 
  m_ebBC->zeroEBBCSource();
}
      ...
      int                         a_iterations)
{
  ...
  if (m_relaxType == 0)
    ...
    {
      MayDay::Error("EBAMRPoissonOpRZ::bogus relaxtype");
    }
}
//-----------------------------------------------------------------------
void
...
relaxGSRBFast(LevelData<EBCellFAB>&       a_phi,
              ...
              int                         a_iterations)
{
  CH_TIME("EBAMRPoissonOpRZ::relaxGSRBFast");
  ...
  for (int whichIter =0; whichIter < a_iterations; whichIter++)
    {
      ...
      for (DataIterator dit = a_phi.dataIterator(); dit.ok(); ++dit)
        {
          ...
          {
            CH_TIME("EBAMRPoissonOpRZ::levelGSRB::applyDomainFlux");
      ...
      for (int redBlack =0; redBlack <= 1; redBlack++)
        {
          CH_TIME("EBAMRPoissonOpRZ::levelGSRB::Compute");

          a_phi.exchange();

          if (m_hasCoar)
            {
              CH_TIME("EBAMRPoissonOpRZ::levelGSRB::homogeneousCFInterp");
...
relaxGauSai(LevelData<EBCellFAB>&       a_phi,
            ...
            int                         a_iterations)
{
  CH_TIME("EBAMRPoissonOpRZ::relaxGauSai");
...
relaxPoiJac(LevelData<EBCellFAB>&       a_phi,
            ...
            int                         a_iterations)
{
  CH_TIME("EBAMRPoissonOpRZ::relaxPoiJac");
...
{
  CH_TIME("EBAMRPoissonOpRZ::gsrbColor");
...
{
  CH_TIME("EBAMRPoissonOpRZ::restrictResidual");
...
{
  CH_TIME("EBAMRPoissonOpRZ::prolongIncrement");
  Interval vars(0, 0);
  if (m_layoutChanged)
    {
...
{
  CH_TIMERS("EBAMRPoissonOpRZ::applyCFBCs");
...
{
  CH_TIME("EBAMRPoissonOpRZ::applyHomogeneousCFBCs");
  CH_assert(a_phi.nComp() == 1);
                      ...
                      Side::LoHiSide        a_hiorlo)
{
  if (m_hasCoar)
    {
      CH_TIMERS("EBAMRPoissonOpRZ::applyHomogeneousCFBCs2");
...
{
  CH_TIMERS("EBAMRPoissonOpRZ::AMRResidual");
  ...
  //  dumpLevelPoint(a_residual, string("EBAMRPoissonOpRZ: AMRResidual: lphi = "));
  //  dumpLevelPoint(a_rhs,      string("EBAMRPoissonOpRZ: AMRResidual: rhs = "));
  //multiply by -1 so a_residual now holds -L(phi)
  //add in rhs so a_residual = rhs - L(phi)
  CH_START(t2);
  axby(a_residual,a_residual,a_rhs,-1.0, 1.0);
  CH_STOP(t2);
  //  dumpLevelPoint(a_residual, string("EBAMRPoissonOpRZ: AMRResidual: resid = "));
}
//-----------------------------------------------------------------------
void EBAMRPoissonOpRZ::
...
AMROperator(LevelData<EBCellFAB>&       a_LofPhi,
            ...
            AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
  CH_TIMERS("EBAMRPoissonOpRZ::AMROperator");
  CH_TIMER("applyOp", t1);
  CH_TIMER("reflux", t2);
  ...
  //  dumpLevelPoint(a_LofPhi, string("EBAMRPoissonOpRZ: AMROperator: pre-reflux lphi = "));
  ...
  //  dumpLevelPoint(a_LofPhi, string("EBAMRPoissonOpRZ: AMROperator: post-reflux lphi = "));
}
//-----------------------------------------------------------------------
void
EBAMRPoissonOpRZ::
reflux(LevelData<EBCellFAB>& a_residual,
       ...
       AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
  CH_TIMERS("EBAMRPoissonOpRZ::fastReflux");
...
incrementFRCoar(EBFastFR&             a_fluxReg,
                ...
                const LevelData<EBCellFAB>& a_phi)
{
  CH_TIME("EBAMRPoissonOpRZ::incrementFRCoar");
...
{
  CH_TIME("EBAMRPoissonOpRZ::getFluxEBCF");
  ...
  if (!ivsCell.isEmpty())
    {
      ...
      for (FaceIterator faceit(ivsCell, a_ebisBox.getEBGraph(), a_idir,stopCrit);
          faceit.ok(); ++faceit)
        {
          ...
          //            {
          //              pout() << "EBAMRPoissonOpRZ::getFlux at "<< face ;
...
incrementFRFine(EBFastFR&             a_fluxReg,
                ...
                AMRLevelOp<LevelData<EBCellFAB> >* a_finerOp)
{
  CH_TIME("EBAMRPoissonOpRZ::incrementFRFine");
  CH_assert(a_phiFine.nComp() == 1);
  CH_assert(a_phi.nComp() == 1);
  CH_assert(m_hasFine);
  int ncomp = 1;
  Interval interv(0,0);
  EBAMRPoissonOpRZ& finerEBAMROp = (EBAMRPoissonOpRZ& )(*a_finerOp);
...
AMRRestrict(LevelData<EBCellFAB>&       a_resCoar,
            ...
            bool a_skip_res )
{
  CH_TIME("EBAMRPoissonOpRZ::AMRRestrict");
  CH_assert(a_residual.ghostVect() == m_ghostCellsRHS);
...

{
  ...
  CH_TIME("EBAMRPoissonOpRZ::AMRNorm");
...
AMRProlong(LevelData<EBCellFAB>&       a_correction,
           const LevelData<EBCellFAB>& a_coarCorrection)
{
  CH_TIME("EBAMRPoissonOpRZ::AMRProlong");
...
AMRUpdateResidual(LevelData<EBCellFAB>&       a_residual,
                  ...
                  const LevelData<EBCellFAB>& a_coarCorrection)
{
  CH_TIME("EBAMRPoissonOpRZ::AMRUpdateResidual");
  CH_assert(a_residual.ghostVect() == m_ghostCellsRHS);
  CH_assert(a_correction.ghostVect() == m_ghostCellsPhi);
  CH_assert(a_coarCorrection.ghostVect() == m_ghostCellsPhi);
