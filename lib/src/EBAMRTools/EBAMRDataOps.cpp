				   ...
				   const EBIndexSpace* const        a_ebisPtr)
{
  ...
  EBQuadCFInterp quadCFIWithFine(a_eblg[a_fineLevel].getDBL(),
                                 ...
                                 a_eblg[coarseLevel].getDomain(),
                                 a_refRatio[coarseLevel],
                                 ncomp, *a_eblg[a_fineLevel].getCFIVS(),a_ebisPtr);
                               ...
                               const Vector<int>&                             a_refRatio)
{
  ...
  // do average down here
  int numLevels = Min(Min(a_data.size(),a_dbl.size()),a_ebisl.size());
  const EBIndexSpace* ebis = a_ebisl[0].getEBIS();
  for (int ilev = (numLevels - 1); ilev>0; ilev--)
    {
      int ncomp = a_data[ilev]->nComp();
                               ...
                               const Vector<int>&                             a_refRatio)
{
  ...
  // do average down here
  int numLevels = Min(Min(a_data.size(),a_eblg.size()),a_refRatio.size());
  const EBIndexSpace* ebis = a_eblg[0].getEBIS();
  for (int ilev = (numLevels - 1); ilev>0; ilev--)
    {
      int ncomp = a_data[ilev]->nComp();
...
//lucaadd
void EBAMRDataOps::setVal(Vector<RefCountedPtr<LevelData<EBFluxFAB> > >& a_result,
                          const Real&                     a_value)
{
  int numLevels = a_result.size();
  for (int ilev = 0; ilev < numLevels; ilev++)
...
//lucaadd
void EBAMRDataOps::setVal(Vector<RefCountedPtr<LevelData<EBCellFAB> > >& a_result,
                          const Real&                     a_value)
{
  int numLevels = a_result.size();
  for (int ilev = 0; ilev < numLevels; ilev++)
...
//lucadd
void EBAMRDataOps::scale(Vector<RefCountedPtr<LevelData<EBFluxFAB> > >& a_result,
                         const Real&                     a_value)
{
  CH_TIME("EBAMRDataOps::scale(flux_result,scale)");
  int numLevels = a_result.size();
                             ...
                             const int&                            a_comp)
{
  ...
  for (int ilev = 0; ilev < numLevels; ilev++)
    {
      if(a_data[ilev] == NULL) continue;
      if(!a_data[ilev]->isDefined()) continue;
      Real maxThis = -1.e99;
      Real minThis =  1.e99;
      EBLevelDataOps::getMaxMin(maxThis,minThis,*a_data[ilev],a_comp);
      a_maxVal = Max(maxThis,a_maxVal);
...
void EBAMRDataOps::getMaxMin(Vector<Real>&                         a_maxVal,
                             Vector<Real>&                         a_minVal,
                             const Vector<LevelData<EBCellFAB>* >& a_data)
{
  //this function gets the max and min values
  int nComp = (*a_data[0]).nComp();
  a_maxVal.resize(nComp);a_minVal.resize(nComp);
  ...
  for (int ilev = 0; ilev < numLevels; ilev++)
      ...
      for (int k = 0; k < nComp; k++)
	{
	  Real maxThis = -1.e99;
	  Real minThis =  1.e99;
	  EBLevelDataOps::getMaxMin(maxThis,minThis,*a_data[ilev],k);
	  a_maxVal[k] = Max(maxThis,a_maxVal[k]);
	  a_minVal[k] = Min(minThis,a_minVal[k]);
	}
    }
}
                               ...
                               const Real&                           a_shift)
{
  ...
  const bool verbosity = true; //lucachanged
          ...
          for (DataIterator dit=a_data[ilev]->dataIterator();dit.ok();++dit)
            {
              ...
              const Box& region = dataEBFAB.getRegion();
              const Box& box = (*a_data[ilev]).box(dit());
              ...
              for (VoFIterator vofit(ivsBox, ebisBox.getEBGraph());vofit.ok(); ++vofit)
                {
                  ...
                  if (std::isnan(val) || std::isinf(val) || Abs(val)>1.e40)
                    {
                      pout() << "     ilev = " << ilev << " icomp = " << icomp << " vof = " << vof << " val = " << val 
			     << "iscovered " << ebisBox.isCovered(vof.gridIndex()) << "isIrregular " << ebisBox.isIrregular(vof.gridIndex()) << 
			" in Box(noghost) " << box.contains(vof.gridIndex()) <<
			" Location " << EBArith::getVofLocation(vof,RealVect::Unit,RealVect::Zero) << std::endl;
                      dataIsNANINF = true;
                    }
      ...
      if (verbosity)
        {
          ...
          pout() << " comp# " << icomp << " max= " << maxVal;
          pout() << " min= " << minVal << ";" ;
	  if(abs(maxVal) > 1e40 || abs(minVal) > 1e40)
	    {
	      pout() << "Setting NANinf to true becasue of maxmin cond" << std::endl;
	      dataIsNANINF = true;
	    }
        }
    }
...
//lucaadd
bool EBAMRDataOps::checkNANBOX(const Vector<LevelData<EBCellFAB>* >& a_data,
                               const IntVect&                        a_iv1,
                               const IntVect&                        a_iv2,
                               const Real&                           a_shift)
{
  ...
  //this function checks for nans and infs only in the boxes
  bool dataIsNANINF = false;
  int numLevels = a_data.size();
  int ncomp = a_data[0]->nComp();
  for (int icomp = 0; icomp < ncomp;++icomp)
      ...
      for (int ilev = 0; ilev < numLevels; ilev++)
        {
          ...
          for (DataIterator dit=a_data[ilev]->dataIterator();dit.ok();++dit)
            {
              const EBCellFAB& dataEBFAB = (*a_data[ilev])[dit()];
              const Box& region = (*a_data[ilev]).box(dit());
              IntVectSet ivsBox(region);
              const EBISBox& ebisBox = dataEBFAB.getEBISBox();
              for (VoFIterator vofit(ivsBox, ebisBox.getEBGraph());vofit.ok(); ++vofit)
                {
...
//lucaadd
void EBAMRDataOps::updateGhost(Vector<LevelData<EBCellFAB>* >&  a_phi, const Real& a_limit,
			       const Vector<EBLevelGrid>&       a_eblg,
			       const Vector<int>&               a_refRatio)
{
  ...
  for (int ilev = 0; ilev < numLevels; ilev++)
      ...
      for (DataIterator dit = dbl.dataIterator(); dit.ok(); ++dit)
	{
	  ...
	  Box bbox = dbl.get(dit());
	  IntVectSet ivs(cbox);
	  ivs -= bbox; // only ghost
	  for (VoFIterator vofit(ivs, ldata[dit()].getEBISBox().getEBGraph()); vofit.ok(); ++vofit)
	    {
	      const VolIndex& vof = vofit();
	      for (int icomp = 0; icomp < pvars; icomp++)
		{
  ...
  EBAMRDataOps::quadCFInterpAll(a_phi,a_eblg,a_refRatio);
  EBAMRDataOps::pwlFillPatchAll(a_phi,a_eblg,a_refRatio);
  EBAMRDataOps::averageDown(a_phi,a_eblg,a_refRatio);
}
