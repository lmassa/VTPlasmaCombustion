...
void
writeEBHDF5noJunk(const string& a_filename,
            const Vector<DisjointBoxLayout>& a_vectGrids,
            const Vector<LevelData<EBCellFAB>* > & a_vectData,
            const Vector<string>& a_vectNames,
            const ProblemDomain& a_domain,
            ...
            IntVect a_ghostVect)
{
  ...
  for (int i = 0; i < a_vectNames.size(); i++) names[i] = a_vectNames[i];
  ...
  for (int ilev = 0; ilev < a_numLevels; ilev++)
    {
      ...
      const LevelData<EBCellFAB>& ebcfData1 = *a_vectData[ilev];
      ...
      chomboData[ilev] = new LevelData<FArrayBox>(grids, ncompTotal, ghostIV);
