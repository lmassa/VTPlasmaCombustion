          ...
          for (vofit.reset(); vofit.ok(); ++vofit)
            {
              ...
              //we increment the coarse side so we have to get the faces on the
              //flip of sd
              Vector<FaceIndex> faces = ebisBox.getFaces(vof, a_dir, flip(a_sd));
              for (int iface = 0; iface < faces.size(); iface++)
                {
                  const FaceIndex& face = faces[iface];
                  for (int ivar = a_varSrc.begin(); ivar <= a_varSrc.end(); ivar++)
                    {
		      int jvar = a_varDst.begin() + (ivar-a_varSrc.begin());
                      ...
                      areaSum *= -a_scale;
                      m_delUCoar[a_coarDatInd](vof, jvar) +=  areaSum;
                  ...
                  const Side::LoHiSide& a_sd)
{
  ...
  /// incrementFineSparse(a_fineFlux, a_scale, a_fineDatInd, a_variables, a_dir, a_sd, true);
  bool doingFineRegular = true;
  for (int k=0; k<2; k++){
  if (m_hasEBCF)
    {
              ...
              for (int ifacef = 0; ifacef < facesFine.size(); ifacef++)
                {
                  ...
                  if (( doingFineRegular &&  vofIsRegular) || (!doingFineRegular && !vofIsRegular))
                    {
                      for (int ivar = a_varSrc.begin(); ivar <= a_varSrc.end(); ivar++)
                        {
			  int jvar = a_varDst.begin() + (ivar-a_varSrc.begin());
                          Real flux  = a_fineFlux(facesFine[ifacef], ivar);
                          delUCoFi(coarVoF, jvar) +=  newScale*area*flux;
                        } //end loop over variables
                    }
