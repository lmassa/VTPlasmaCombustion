          ...
          for (VoFIterator vofit(ivsBox, ebisBox.getEBGraph());vofit.ok(); ++vofit)
            {
              ...
              if (std::isnan(val) || std::isinf(val) || Abs(val)>1.e40)
                {
                  pout() << " icomp = " << icomp << " vof = " << vof << " val = " << val << " In S " << boxS.contains(vof.gridIndex())  << "Isirr " <<  ebisBox.isIrregular(vof.gridIndex()) << std::endl;
                  dataIsNANINF = true;
                }
...
bool EBLevelDataOps::checkNANINF(const LevelData<EBFluxFAB> &a_data)
{
  ...
  for (int icomp = 0; icomp < ncomp;++icomp)
    {
	  ...
	  for (int idir = 0; idir < SpaceDim; ++idir)
	      ...
	      for (FaceIterator faceit(ivsBox, ebisBox.getEBGraph(), idir, stopCritGrid); faceit.ok(); ++faceit)
		{
		  ...
		  if (std::isnan(val) || std::isinf(val) || Abs(val)>1.e40)
		    {
		      pout() << " icomp = " << icomp << " ind low = " << face.gridIndex(Side::Lo) << " val = " << val << std::endl;
		      dataIsNANINF = true;
		    }
		}
	    }
  ...
  for (DataIterator dit=a_data.dataIterator();dit.ok();++dit)
    {
      ...
      for (int idir = 0; idir < SpaceDim; ++idir)
	{
	  Real val1 = a_data[dit()].max(idir,a_comp);
	  Real val2 = a_data[dit()].min(idir,a_comp);
          if (a_doAbs)
            {
              a_maxVal = Max(Max(a_maxVal,Abs(val1)),Abs(val2));
              a_minVal = Min(Min(a_minVal,Abs(val2)),Abs(val1));
            }
          else
            {
              a_maxVal = Max(a_maxVal,val1);
              a_minVal = Min(a_minVal,val2);
            }
	}
    }
  a_minVal = EBLevelDataOps::parallelMin(a_minVal);
...
//---- lucaadded incr with components
void EBLevelDataOps::incr( LevelData<EBCellFAB>& a_lhs,
                           const LevelData<EBCellFAB>& a_rhs,
                           int a_rhscomp,
			   int a_lhscomp,
			   int a_numcomp)
{
  ...
  for (DataIterator dit = a_lhs.dataIterator(); dit.ok(); ++dit)
    {
      DataIndex d = dit();
      a_lhs[d].plus(a_rhs[d], a_rhscomp, a_lhscomp, a_numcomp);
    }
}

//---- lucaadded, with components and scale
void EBLevelDataOps::incr( LevelData<EBCellFAB>& a_lhs,
                           const LevelData<EBCellFAB>& a_rhs,
                           int a_rhscomp,
			   int a_lhscomp,
			   ...
			   Real a_scale)
{
  ...
  for (DataIterator dit = a_lhs.dataIterator(); dit.ok(); ++dit)
    {
      DataIndex d = dit();
      a_lhs[d].plus(a_rhs[d], a_rhscomp, a_lhscomp, a_numcomp, a_scale);
    }
}
//lucaadd FluxFab
void EBLevelDataOps::incr(LevelData<EBFluxFAB>& a_lhs,
                           const LevelData<EBFluxFAB>& a_rhs,
                           int a_rhscomp,
			   int a_lhscomp,
			   int a_numcomp,
  ...
  for (DataIterator dit = a_lhs.dataIterator(); dit.ok(); ++dit)
    {
      ...
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          EBFaceFAB& lhs = a_lhs[d][idir];
          const EBFaceFAB& rhs = a_rhs[d][idir];
	  lhs.plus(rhs, a_rhscomp, a_lhscomp, a_numcomp, a_scale);
        }
    }
}
  ...
  for (DataIterator dit = a_result.dataIterator(); dit.ok(); ++dit)
    {
      DataIndex d = dit();
      EBCellFAB& result = a_result[d];
      result.mult(a_value[dit()], a_valcomp, a_rescomp, a_numcomp);
    }
}
...
//lucaadd
void EBLevelDataOps::subtraction(LevelData<EBCellFAB>&       a_result,
                         const LevelData<EBCellFAB>& a_in1,
                         const LevelData<EBCellFAB>& a_in2)
{
  for (DataIterator dit = a_result.dataIterator(); dit.ok(); ++dit)
...
//lucaadd
void EBLevelDataOps:: ABS(LevelData<EBFluxFAB>& a_data)
{
  CH_TIME("EBLevelDataOps::abs");
  ...
  for (DataIterator dit = a_data.dataIterator(); dit.ok(); ++dit)
	...
	{
	  FaceStop::WhichFaces stopCritGrid =  FaceStop::SurroundingNoBoundary;
	  EBFaceFAB& data = a_data[dit()][idir];
	  const Box& region = data.getCellRegion();
	  IntVectSet ivsBox(region);
	  ...
	  for (FaceIterator faceit(ivsBox, ebisBox.getEBGraph(), idir, stopCritGrid); faceit.ok(); ++faceit)
		...
		{
		  const Real& dataIn = data(face,icomp);
		  data(face,icomp) =abs(dataIn);
		}
	    }
	}
    }
...
void EBLevelDataOps::divideVectorByScalar(LevelData<EBCellFAB>&       a_vectorOut,
                                          const LevelData<EBCellFAB>& a_vectorIn,
                                          const LevelData<EBCellFAB>& a_scalar,
					  const Interval&             a_OutInterval,
					  const Interval&             a_InInterval,
					  const int&                  a_scalComp)
{
  //a_vector /= a_scalar

  int jcomp = a_InInterval.begin();
  for (int icomp = a_OutInterval.begin();icomp<=a_OutInterval.end();++icomp)
    {
      divide(a_vectorOut,a_vectorIn,a_scalar,icomp,jcomp,a_scalComp);
      jcomp++;
    }
}

//lucaadd
void EBLevelDataOps::multiplyVectorByScalar(LevelData<EBCellFAB>&       a_vectorOut,
					    const LevelData<EBCellFAB>& a_vectorIn,
					    const LevelData<EBCellFAB>& a_scalar)
{
  //a_vector *= a_scalar
  ...
  for (int icomp = 0;icomp<nCompVectorOut;++icomp)
    {
      product(a_vectorOut,a_vectorIn,a_scalar,icomp,icomp,0);
    }
}
//lucaadd
void EBLevelDataOps::multiplyVectorByScalar(LevelData<EBCellFAB>&       a_vectorOut,
					    ...
					    const int&                  a_scalComp)
{
  ...
  for (int icomp = a_OutInterval.begin();icomp<=a_OutInterval.end();++icomp)
    {
      product(a_vectorOut,a_vectorIn,a_scalar,icomp,jcomp,a_scalComp);
      jcomp++;
    }
}
...
// lucaadd
void EBLevelDataOps::arrayNorm(LevelData<EBCellFAB>&       a_result,
  ...
  for (DataIterator dit = a_result.dataIterator(); dit.ok(); ++dit)
    {
      ...
      result.arrayNorm(a_src[d], a_scale);
    }
}
...
//lucaadd
//-----------------------------------------------------------------------
void EBLevelDataOps::kappaDivide(LevelData<EBCellFAB>& a_data)
{
  for (DataIterator dit = a_data.dataIterator(); dit.ok(); ++dit)
    {
      DataIndex d = dit();
      ...
      for (VoFIterator vofit(dataIrreg,dataEBGraph); vofit.ok(); ++vofit)
	{
	  ...
	  for (int comp = 0; comp < nComp; comp++)
	    {
	      Real kappa = dataEBISBox.volFrac(vof);
              if(abs(kappa) > 1e-6) data(vof,comp) /= kappa;
	    }
	}
...
Real EBLevelDataOps::pickData(const LevelData<EBCellFAB>&  a_data, const IntVect& a_iv, int a_comp)
{ 
  Real value = -1e199;
  if(a_comp <0 || a_comp>= a_data.nComp()) return value;
  for (DataIterator dit = a_data.dataIterator(); dit.ok(); ++dit)
    {
      Box box = a_data[dit()].getRegion(); //includes ghost cells
      const EBISBox& ebisBox = a_data[dit()].getEBISBox();
      if(box.contains(a_iv)){
	  const VolIndex vof(a_iv,0);
	  value = a_data[dit()](vof, a_comp);
      }
    }
  return value;
}
void EBLevelDataOps::printIV(const LevelData<EBCellFAB>&  a_data, const IntVect& a_iv)
{ 
  int nComp = a_data.nComp();
  for (DataIterator dit = a_data.dataIterator(); dit.ok(); ++dit)
    {
	...
	{
	  const VolIndex vof(a_iv,0);
	  pout() << vof << " IValues found. Is Irregular? " << ebisBox.isIrregular(a_iv);
	  for (int k=0;k<nComp;k++) pout() << a_data[dit()](vof, k) << ", ";
	  pout() << endl;
	}
    }
}
void EBLevelDataOps::printIVFAB(const EBCellFAB&  a_data, const IntVect& a_iv)
{ 
  int nComp = a_data.nComp();
  Box box = a_data.getRegion(); //includes ghost cells
  const EBISBox& ebisBox = a_data.getEBISBox();
  if(box.contains(a_iv))
    {
      const VolIndex vof(a_iv,0);
      pout() << vof << " IValues found. Is Irregular? " << ebisBox.isIrregular(a_iv) << ", ";
      for (int k=0;k<nComp;k++) pout() << a_data(vof, k) << ", ";
      pout() << endl;
    }
}
Real EBLevelDataOps::pickDataFab(const EBCellFAB&  a_data, const IntVect& a_iv, int a_comp)
{ 
  Real value = -1e199;
  if(a_comp <0 || a_comp>= a_data.nComp()) return value;
  Box box = a_data.getRegion(); //includes ghost cells
  ...
  if(box.contains(a_iv)){
    const VolIndex vof(a_iv,0);
    value = a_data(vof, a_comp);
  }
  return value;
}
//lucadd
void
EBLevelDataOps::checkdata(const LevelData<EBCellFAB>&  a_data, const string& a_name)
{  
  ofstream  locout;
  string dname = a_name + string(".dbg");
  ...
  for (int ip=0; ip<numProc(); ++ip)
      ...
      if(procID() == ip)
	{
	  if(ip==0)
	    {
	      locout.open(dname.c_str(),ofstream::trunc);
	      locout << "____________________________" << endl;
	    }
	  else
	    locout.open(dname.c_str(),ofstream::app);
	  ...
	  for (DataIterator dit = a_data.dataIterator(); dit.ok(); ++dit)
	      ...
	      for (VoFIterator vofit(ivs, ebisBox.getEBGraph());vofit.ok(); ++vofit)
		{
		  ...
		  for (int i=0;i<SpaceDim;i++)locout <<  gi[i]<<  ", ";	  
		  for (int k=0;k<nCons;k++) locout << a_data[dit()](vof, k) << ", ";
		  locout << endl;
		}
	    }
	  locout.close();
...
//lucadd
bool
EBLevelDataOps::symmetrize(LevelData<EBCellFAB>&  a_data, const Real& a_domainLength, const Real& a_dx,const int& a_cmomz)
{  

  bool outSymm = true;
  if(SpaceDim != 3) { pout() << "NO symmetrization in 2D" << endl; return outSymm;}
  
  bool checkSymm = a_cmomz<0;
  int sgn;
  int nComp = a_data.nComp();
  int zshift = floor(a_domainLength/2.0/a_dx+0.5);
  for (DataIterator it = a_data.dataIterator(); it.ok(); ++it)
    {
      Box iBox = a_data.box(it());
      Box sBox = a_data.box(it());
      int gshift = 2*zshift-sBox.size(2)-2*sBox.smallEnd(2);
      sBox.shift(2, gshift);
      if(gshift <0)
	{
	  bool boxFound = false;
	  for (DataIterator jt = a_data.dataIterator(); jt.ok(); ++jt)
	    {
	      Box jBox = a_data.box(jt());
	      if(jBox == sBox)
		{
		  EBCellFAB& dest=a_data[it()];
		  EBCellFAB& src = a_data[jt()];
		  Box jBox2 = src.getRegion();
		  IntVectSet ivs( dest.getRegion()  );//includes ghost cells
		  for (VoFIterator vofit(ivs, dest.getEBISBox().getEBGraph());vofit.ok(); ++vofit)
		    {
		      const VolIndex& vof = vofit();
		      IntVect gi = vof.gridIndex();int ci=vof.cellIndex();
		      gi[2] = 2*zshift-gi[2]-1;
		      if(jBox2.contains(gi))
			{
			  VolIndex Svof(gi,ci);
			  bool isSymmetric = true;
			  if(!checkSymm)
			    for (int k=0;k<nComp;k++) dest(vof, k) = src(Svof, k);
			  else
			    {
			      for (int k=0;k<nComp;k++) {
				sgn = (k == abs(a_cmomz)) ? -1e0 : 1e0;
				isSymmetric = isSymmetric & abs(dest(vof, k) - sgn*src(Svof, k)) < 1e-12;
			      }
			      if(!isSymmetric){
				pout() << "SYmm " << vof.gridIndex() <<  ", ";
				for (int k=0;k<nComp;k++) pout() << dest(vof, k)<< ", ";
				pout() << endl;
				pout() << "Diff " << Svof.gridIndex() <<  ", ";
				for (int k=0;k<nComp;k++) pout() << src(Svof, k)-dest(vof, k)<< ", ";
				pout() << endl;
			      }
			      outSymm=outSymm&isSymmetric;
			    }
			}
		    }
		  if(!checkSymm) dest.mult(-1.0,a_cmomz,1);
		  boxFound = true;
		  break;
		}
	    }
	  if(!boxFound) MayDay::Error("EBLevelDataOps::symmetrize::Box Not Found!");
	}
    }
  return (outSymm);
}
/*****/
//lucaAdd
void EBLevelDataOps::averageCellToFacesAll(EBFaceFAB&             a_faceData,
						const EBCellFAB&       a_cellData,
						const EBGraph&         a_ebGraph,
						const Box&             a_dblBox,
						const int&             a_ghostFluxTan,
						...
						const ProblemDomain&   a_domain)
{
  ...
  //do faces inside the domain
  {
    ...
    //do regular cells in fortran for the faces in faceBox
    FORT_EBAVECELLTOFACEALL(CHF_FRA(      regFaceData),
			    CHF_CONST_FRA(regCellData),
			    CHF_CONST_INT(a_idir),
			    CHF_BOX(faceBox));
	...
	for(int icomp=0; icomp< numComp; icomp++)
	  a_faceData(face, icomp) = 0.5*(a_cellData(face.getVoF(Side::Hi), icomp) +
					      a_cellData(face.getVoF(Side::Lo), icomp));
      }
  }
          ...
          for (BoxIterator boxit(edge); boxit.ok(); ++boxit)
            {
              IntVect ivCell = boxit();
              IntVect ivFace = boxit();
              if (outSide() == Side::Hi) ivFace.shift(BASISV(a_idir));
	      for(int icomp=0; icomp< numComp; icomp++)
		regFaceData(ivFace, icomp) = regCellData(ivCell, icomp);
            }
	      ...
	      for(int icomp=0; icomp< numComp; icomp++){
		const Real&     value = a_cellData(vof, icomp);
	      
		if (faces.size() == 1)
		  {//linear extrapolation
		    const VolIndex&   vofNeigh = faces[0].getVoF(inSide);
		    const Real&     valueNeigh = a_cellData(vofNeigh, icomp);
		    a_faceData(face, icomp) = 0.5*(3.0*value - valueNeigh);
		  }
		else
		  {//zero order extrapolation for != 1 inside faces
		    a_faceData(face, icomp) = value;
		  }
	      }
            }
        }
...
/*****/
void EBLevelDataOps::averageCellToFacesAll(LevelData<EBFluxFAB>&         a_fluxData,
                                        const LevelData<EBCellFAB>&   a_cellData,
                                        const DisjointBoxLayout&      a_grids,
                                        const EBISLayout&             a_ebisl,
                                        const ProblemDomain&          a_domain)
  ...
  for (DataIterator dit = a_grids.dataIterator(); dit.ok(); ++dit)
    {
      ...
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          EBFaceFAB&       faceData = fluxData[idir];
          averageCellToFacesAll(faceData, cellData, ebgraph, dblBox, ghostFluxTan, idir, a_domain);
        }
    }
  exchangeAll(a_fluxData);
}
