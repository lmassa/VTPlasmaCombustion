...
//  CH_START(t1);
  ...
  //lucaChange below
  int nvarLM = a_varDest;
  for (int isrc = 0; isrc < a_srcVofs.size(); isrc++)
    {
      const VoFStencil& sten = a_vofStencil[isrc];
      for (int isten = 0; isten < sten.size(); isten++)
        {
          nvarLM = Max(sten.variable(isten),nvarLM);
	}
    }

  BaseIVFAB<Real> baseivfabPhi(ivsPhi, ebgraphPhi, nvarLM+1);
  BaseIVFAB<Real> baseivfabLph(ivsLph, ebgraphLph, a_varDest+1);
...
#else
          m_destTerms[isrc].offset += m_destVar*ncellsLph[0];
          bogus_spacedim();
#endif
...
#else
              stenEntry.offset += srcVar*ncellsPhi[0];
              bogus_spacedim();
#endif
                          ...
                          const BaseIVFAB<VoFStencil>& a_vofStencil)
{
  ...
  BaseIVFAB<Real> baseivfabPhi(ivsPhi, ebgraph, nvarLM+1);
  BaseIVFAB<Real> baseivfabLph(ivsLph, ebgraph, m_destVar+1);


  if (m_doRelaxOpt)
    {
      m_alphaBeta.resize(a_srcVofs.size());
      const IntVectSet& ivsIrr = m_ebisBox.getIrregIVS(m_box);
...
#else
          m_destTerms[isrc].offset += m_destVar*ncellsLph[0];
          m_sourTerms[isrc].offset += m_destVar*ncellsPhi[0];
          bogus_spacedim();
#endif
          m_destTerms[isrc].multiValued = false;
        }
