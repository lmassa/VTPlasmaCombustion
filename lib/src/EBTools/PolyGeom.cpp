...
#ifdef CH_USE_FLOAT
Real PolyGeom::s_tolerance = 1.0e-6;
Real PolyGeom::s_areatolerance = 1.0e-4;
Real PolyGeom::s_lengthtolerance = 1.0e-2;
#else
...
invertMatrix(Real a_AInverse[SpaceDim][SpaceDim], const Real a_A[SpaceDim][SpaceDim], bool a_test)
{
  ...
  if (Abs(detA) < 1.0e-10)
    {
      MayDay::Error("invertMatrix: determinant appears to be zero");
    }
                 ...
                 const Real& a_bndryArea)
{
  ...
  if (a_ebisBox.isRegular(iv))
    ...
    {
      CH_assert(a_ebisBox.isIrregular(iv));
                          ...
                          const int& a_icomp)
{
  ...
  Real eps = 1.0e-10;
  if (Abs(denom) < eps)
    MayDay::Error("MatrixSolveComp has encountered unsolvable matrix");
...
determinantSD(const Real a_A[SpaceDim][SpaceDim])
{
  ...
  D_SELECT6(
    ...
    MayDay::Error("PolyGeom::Determinant not equiped for your fancy 4 dimensions yet");,

    MayDay::Error("PolyGeom::Determinant not equiped for your fancy 5 dimensions yet");,

    MayDay::Error("PolyGeom::Determinant not equiped for your fancy 6 dimensions yet");
  );
                       ...
                       const RealVect& a_normal)
{
  ...
  CH_assert(a_volFrac >= 0.0);
  CH_assert(a_volFrac <= 1.0);
                    ...
                    const Real& a_alpha)
{
  ...
  for (int idir = 0; idir < SpaceDim; idir++)
    {
      CH_assert(a_normal[idir] > s_tolerance);
