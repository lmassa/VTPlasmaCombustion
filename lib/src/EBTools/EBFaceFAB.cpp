...
EBFaceFAB::EBFaceFAB(const EBFaceFAB& a_ebcin, int a_newComps):BaseEBFaceFAB<Real>()
{
  define(a_ebcin.getEBISBox(), a_ebcin.getCellRegion(), a_ebcin.direction(), a_newComps);
}
          ...
          if (locRegionFace.contains(face.gridIndex(Side::Hi)))
            {
              for (int icomp = 0; icomp < a_numcomp; ++icomp)
                {
                  m_irrFAB(face, a_destcomp+icomp) +=
                    a_src.m_irrFAB(face, a_srccomp+icomp) * a_scale;
                }
            }
        }
    }
  ...
  if (!locRegion.isEmpty())
    {
      ...
      for (int iface = 0; iface < faces.size(); iface++)
        {
          const FaceIndex& face = faces[iface];
          if (locRegionFace.contains(face.gridIndex(Side::Hi)) && a_src.m_irrFAB.containsIndex(face))
            {
              for (int icomp = 0; icomp < a_numcomp; ++icomp)
                {
                  m_irrFAB(face, a_destcomp+icomp) *=
