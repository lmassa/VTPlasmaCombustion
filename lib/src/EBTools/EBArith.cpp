...
{
  IntVect iv0 = a_vof.gridIndex();
  IntVect ivDbg= IntVect(D_DECL(64,17,16));
  if (iv0 == ivDbg)
    {
      // debug hook
      iv0 = iv0;
  ...
  int stenSize = 7;
#else
  int stenSize = 3;// just to compile
  MayDay::Error("getLeastSquaresGradSten doesnt work in 1D;");
#endif
  Box domainBox = a_domain.domainBox();
                               ...
                               int                  a_ivar)
{
  ...
  int numNeighbors = 8;
  int minStenSize  = 3;
  MayDay::Error("getLeastSquaresGradStenAllVoFs doesnt work in 1D;");
#endif
  Box domainBox = a_domain.domainBox();
  Vector<IntVect> ivSten(numNeighbors);
                        ...
                        EBNormType::NormMode a_mode)
{
  ...
  int idoirr = 0;
  int idoreg = 0;
  //lucanote a_mode defaulted to OverBoth
...
#else
  MayDay::Error("getInterpStencil doesnt work in 1D;");
