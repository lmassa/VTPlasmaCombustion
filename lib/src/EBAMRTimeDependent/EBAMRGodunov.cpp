...
/***************************/
int EBAMRGodunov::getRedistRadius()
{
  return(m_redistRad);
}
/***************************/
...
/***************************/
void EBAMRGodunov::setQbar(LevelData<EBCellFAB>*  a_qbar,Real a_chi)
{
...
void EBAMRGodunov::readCheckpointLevel(HDF5Handle& a_handle)
{
  ...
  if(nCellsArray[2] == 2*nCellsArray[1])
    {
      pout() << "Symmetrizing the Init file" << endl;
      readCheckpointLevelSymm(a_handle);
      return;
    }
...
/***************************/
void EBAMRGodunov::readCheckpointLevelSymm(HDF5Handle& a_handle, int cmomz)
{
  CH_assert(m_isDefined);
  if (s_verbosity >= 3)
    {
  ...
  int zshift = floor(m_domainLength[2]/2.0/m_dx[2]+0.5);
  Box old_box(m_domainBox);
  old_box.growHi(2,-zshift);

  Vector<int> proc_map;
  if (s_isLoadBalanceSet)
    {
      s_loadBalance(proc_map,vboxGrids, old_box, false);
    }
  else
    {
      LoadBalance(proc_map,vboxGrids);
  ...
  DisjointBoxLayout old_grids(vboxGrids,proc_map);

  //the ebisl has to know about the fact that we really have
  //four ghost cells.
  int nGhostEBISL = 6;
  const EBIndexSpace* const ebisPtr = Chombo_EBIS::instance();
  EBISLayout old_ebisl;
  ebisPtr->fillEBISLayout(old_ebisl, old_grids, m_domainBox, nGhostEBISL);
  EBCellFactory factoryOld(old_ebisl);
  //m_nghost is set in define function
  IntVect ivGhost = m_nGhost*IntVect::Unit;
  LevelData<EBCellFAB> old_state;
  old_state.define(old_grids,m_nComp, ivGhost, factoryOld);
  
  int dataStatusNew = read<EBCellFAB>(a_handle, old_state, "dataNew", old_grids, Interval(), false);

  int isizeOld = vboxGrids.size();
  vboxGrids.resize(isizeOld*2);
  proc_map.resize(isizeOld*2);
  for (int k=isizeOld; k<vboxGrids.size(); k++)
    {
      vboxGrids[k] =vboxGrids[k-isizeOld];
      int gshift = 2*zshift-vboxGrids[k].size(2)-2*vboxGrids[k].smallEnd(2);
      vboxGrids[k].shift(2,gshift);
      proc_map[k] = proc_map[k-isizeOld];
    }
  m_grids= DisjointBoxLayout(vboxGrids,proc_map);
  ...
  if (dataStatusNew != 0) MayDay::Error("file does not contain state data");


  for (DataIterator it = m_stateNew.dataIterator(); it.ok(); ++it)
    {
      Box iBox = m_stateNew.box(it());
      Box sBox = m_stateNew.box(it());
      int gshift = 2*zshift-sBox.size(2)-2*sBox.smallEnd(2);
      sBox.shift(2, gshift);
      bool boxFound = false;
      for (DataIterator jt = old_state.dataIterator(); jt.ok(); ++jt)
	{
	  Box jBox = old_state.box(jt());
	  if(gshift <0 && jBox == sBox)
	    {
	      EBCellFAB& dest=m_stateNew[it()];
	      EBCellFAB& src = old_state[jt()];
	      ...
	      for (VoFIterator vofit(ivs, dest.getEBISBox().getEBGraph());vofit.ok(); ++vofit)
		{
		  ...
		  if(SpaceDim == 3) gi[2] = 2*zshift-gi[2]-1;
		  if(jBox2.contains(gi))
		    {
		      VolIndex Svof(gi,ci);
		      for (int k=0;k<m_nComp;k++) dest(vof, k) = src(Svof, k);
		    }
		}
	      dest.mult(-1.0,cmomz,1);
	      boxFound = true;
	      break;
	    }
	  else if(jBox == iBox)
	    {	      
	      m_stateNew[it()].copy(old_state[jt()]);
	      boxFound = true;
	      break;
	    }
	}
      if(!boxFound) MayDay::Error("Box Not Found!");
      m_stateOld[it()].copy(m_stateNew[it()]);
    }
  // Set up data structures
  if(false){
    Vector<Box> newGrids = m_grids.boxArray();
    regrid(newGrids);
  }
  else
    levelSetup();
...
{  
  ...
  if(!m_out.is_open()){  
    ...
    Real peps = 1e-4;
    char charstr[100];
    sprintf(charstr, "orig%d.lvl", procID());
    m_out.open(charstr);m_dumps=0;
    m_out.precision(9);
    //m_out << "From original code " <<  procID() <<endl;
    m_NAN = 0e0;
  }
  m_out << "%DT= " <<  m_dt <<endl;
  const LevelData<EBCellFAB>&  AD = m_stateOld;
  const LevelData<EBCellFAB>&  BD = m_stateNew;
  int nCons = Min(AD.nComp(),5);  // to avoid checking all the primitive comps  
  for (DataIterator dit = AD.dataIterator(); dit.ok(); ++dit)
    {
      Box box = m_grids.get(dit());
      ...
      for (VoFIterator vofit(ivs, ebisBox.getEBGraph());vofit.ok(); ++vofit)
	{
	  ...
	  m_out  << a_msg << m_level << ", "<< m_dumps << ", " << gi[0] << ", "<< gi[1] << ", "<< gi[2] <<  ", ";		  
	  for (int k=0;k<nCons;k++) m_out << BD[dit()](vof, k)<< ", ";	  
	  //for (int k=0;k<nCons;k++) m_out << (BD[dit()](vof, k)- AD[dit()](vof, k))/m_dt << ", ";
	  m_out << endl;
	}
    }
  if(m_dumps > 6){
