...
bool EBPatchGodunov::s_conservativeSource  = true;
            ...
            const Real&       a_timestep)
{
  ...
  m_primMinuTemp.define(m_ebisBox, m_validBoxG4, numPrim);  //used in do111coupling, only
  m_primPlusTemp.define(m_ebisBox, m_validBoxG4, numPrim);  //used in do111coupling, only
...
regularUpdate(EBCellFAB&          a_consState,
              ...
              const EBCellFAB&    a_flattening,
              const EBCellFAB&    a_source,
              const Box&          a_box,  // cellBox in EBLeveloGdunov calling program
              const IntVectSet&   a_ivsSmall, // ivsIrreg=m_irregSetsSmall[dit()] in EBLevelGdu
...
regularDivergences(EBCellFAB&          a_nonConsDivF,
                   ...
                   bool                a_verbose)
{
  CH_TIME("EBPatchGodunov::regularDivergences");
  CH_assert(isDefined());
  int numCons = numConserved();
...
void
EBPatchGodunov::
checkFAB(const EBCellFAB&  a_data,  const DataIndex&  a_dit, const string& a_msg)
{
  int nCons = Min(a_data.nComp(),numConserved());  // to avoid checking all the primitive comps
  Box box = m_validBox;
  ...
  for (VoFIterator vofit(ivs, ebisBox.getEBGraph());vofit.ok(); ++vofit)
    {
      ...
      bool dout = false;
      for (int k=0;k<nCons;k++) dout = dout || ! (abs(a_data(vof, k))<m_NAN);
      if (dout){
	m_out << a_msg << 0*floor(m_time/m_dt +0.5) << ", "<< gi[0] << ", "<< gi[1] << ", "<< gi[2] <<  ", ";		  
...
void
EBPatchGodunov::
checkFAB(const EBCellFAB&  a_data,  const string& a_msg)
{
  int nCons = Min(a_data.nComp(),numConserved());  // to avoid checking all the primitive comps
  Box box = m_validBox;
  //Box box = a_data.getRegion(); //includes ghost cells
...
void
EBPatchGodunov::
checkFAB(const EBCellFAB&  a_data,  const Box& a_box, const string& a_msg)
{
...
void
EBPatchGodunov::
checkFAB(const EBCellFAB  a_data[SpaceDim],  const DataIndex&  a_dit, const string& a_msg)
{
  //set up the data structures
  for (int idir = 0; idir < SpaceDim; ++idir)
    {      
      int nCons = Min(a_data[idir].nComp(),numConserved());  // to avoid checking all the primitive comps
      Box box = m_validBox;
      //Box box = a_data[idir].getRegion(); //includes ghost cells
      const EBISBox& ebisBox = a_data[idir].getEBISBox();
      IntVectSet ivs(box);
      for (VoFIterator vofit(ivs, ebisBox.getEBGraph());vofit.ok(); ++vofit)
	{
	  const VolIndex& vof = vofit();
	  IntVect gi = vof.gridIndex();
	  bool dout = false;
	  for (int k=0;k<nCons;k++) dout = dout || ! (abs(a_data[idir](vof, k))<m_NAN);
	  if (dout){
	    m_out << a_msg <<0*floor(m_time/m_dt +0.5) << ", " << idir << ", " << gi[0] << ", "<< gi[1] << ", "<< gi[2] <<  ", ";		  
	    for (int k=0;k<nCons;k++) m_out << a_data[idir](vof, k) << ", ";
	    m_out << endl;
	  }
	}
    }
...
void
EBPatchGodunov::
checkFAB(const EBFluxFAB& a_data,  const DataIndex&  a_dit, const string& a_msg)
{
  //set up the data structures
  bool dataIsNANINF = false;
  int nCons = Min(a_data.nComp(),numConserved());  // to avoid checking all the primitive comps
  ...
  for (int idir = 0; idir < SpaceDim; ++idir)
    {
      const EBFaceFAB& dataFaceFAB = a_data[idir];
      const EBISBox& ebisBox = dataFaceFAB.getEBISBox();
      IntVectSet ivs(box);
      for (FaceIterator faceit(ivs, ebisBox.getEBGraph(), idir, stopCritGrid); faceit.ok(); ++faceit)
	{
	  const FaceIndex& face = faceit();
	  IntVect gi = face.gridIndex(Side::Lo);
	  bool dout = false;
	  for (int k=0;k<nCons;k++) dout = dout || ! (abs(dataFaceFAB(face, k))<m_NAN);
	  if (dout){
	    m_out << a_msg <<0*floor(m_time/m_dt +0.5) << ", " << idir << ", " << gi[0] << ", "<< gi[1] << ", "<< gi[2] <<  ", ";		  
	    for (int k=0;k<nCons;k++) m_out << dataFaceFAB(face, k) << ", ";
	    m_out << endl;
	    dataIsNANINF = true;
	  }
	}
...
void
EBPatchGodunov::
checkFAB(const EBFaceFAB& a_data, const DataIndex&  a_dit, const int a_idir, const string& a_msg)
{
  //set up the data structures
  bool dataIsNANINF = false;
  int nCons = Min(a_data.nComp(),numConserved());  // to avoid checking all the primitive comps
  ...
  for (FaceIterator faceit(ivs, ebisBox.getEBGraph(), a_idir, stopCritGrid); faceit.ok(); ++faceit)
    {
      ...
      for (int k=0;k<nCons;k++) dout = dout || ! (abs(dataFaceFAB(face, k))<m_NAN);
      if (dout){
	m_out << a_msg <<0*floor(m_time/m_dt +0.5) << ", " << a_idir << ", " << gi[0] << ", "<< gi[1] << ", "<< gi[2] <<  ", ";		  
	for (int k=0;k<nCons;k++) m_out << dataFaceFAB(face, k) << ", ";
	m_out << endl;
	dataIsNANINF = true;
      }
...
void
EBPatchGodunov::
checkFAB(const EBFaceFAB& a_data, const int a_idir, const string& a_msg)
{
  //set up the data structures
  bool dataIsNANINF = false;
  int nCons = Min(a_data.nComp(),numConserved());  // to avoid checking all the primitive comps
...
void
EBPatchGodunov::
checkFAB(const EBFaceFAB a_data[SpaceDim], const string& a_msg)
{
  //set up the data structures
  bool dataIsNANINF = false;
  for (int idir = 0; idir < SpaceDim; ++idir)
...
void
EBPatchGodunov::
checkFAB(const BaseIVFAB<Real>&  a_data, const string& a_msg)
{
  //set up the data structures
  bool dataIsNANINF = false;
  int nCons = Min(a_data.nComp(),numConserved());  // to avoid checking all the primitive comps
