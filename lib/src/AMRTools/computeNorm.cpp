          ...
          const int&                    a_p)
{
  ...
  if (a_p == 0)
  {
    for (dit.reset(); dit.ok(); ++dit)
    {
      ...
      {
        norm = Max(norm, thisFlux[dir].norm(validRegion[dit()],
                                            0,
                                            a_interval.begin(),
                                            a_interval.size()));
      }
    ...
    if (result != MPI_SUCCESS)
    {
      MayDay::Error("sorry, but I had a communcation error in norm");
    }
  ...
  else if (a_p == 1) // abs sum norm
  {
    for (dit.reset(); dit.ok(); ++dit)
    {
      ...
      for (int dir = 0; dir < SpaceDim; ++dir)
      {
        norm += thisFlux[dir].norm(validRegion[dit()],
                                   1,
                                   a_interval.begin(),
                                   a_interval.size());
      }
    ...
    for (dit.reset(); dit.ok(); ++dit)
      {
        ...
        for (int dir = 0; dir < SpaceDim; dir++)
        {
          norm+=thisFlux[dir].sumPow(validRegion[dit()],
                                     a_p,
                                     a_interval.begin(),
                                     a_interval.size());
        }
