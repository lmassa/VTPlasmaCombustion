...
void AMR::setDefaultValues()
{
  ...
  m_viscousAssign = false;
  m_viscousAnalysis   = false;
}
//-----------------------------------------------------------------------
                                    ...
                                    int                         a_proper_nest)
{
...

  {
	...
	if (m_verbosity >= 2) pout() << " computeInitialDt m_dt_new of " << level << " = " << m_dt_new[level] << endl;
      }
...
int AMR::timeStep(int a_level, int a_stepsLeft, bool a_coarseTimeBoundary)
{
  ...
  if (m_verbosity >= 3)
    {
      pout() << "AMR::timeStep(" << a_level << ")" << " useSubcycling " <<  m_useSubcycling << endl;
    }
  ...
  if (m_useSubcycling)
    {
      ...
      if ((!a_coarseTimeBoundary) && (m_fixedDt <= 0))
        {
          ...
          // for (int i = a_level; i <= m_max_level; i++)  //lucacancel changed this to the one below
          for (int i = a_level; i <= m_finest_level; i++)
            {
              int factor;
              Real dtCur = m_amrlevels[i]->dt();
              Real dtNew = m_dt_new[i];
	      if (m_verbosity >= 2) pout() << " subcycle m_dt_new of " << i << " = " << m_dt_new[i] << "fnest level " << m_finest_level << " maxlevel " << m_max_level << endl;
	      ...
	      int nReductions = 0, maxReductions=1;
              while (dtCur > m_dt_tolerance_factor*dtNew)
                {
		  if (m_verbosity >= 1)pout ()<< i << " Reducing the time step dtNew " << dtNew << endl;
                  factor *= 2;
                  dtCur *= 0.5;
		  nReductions ++;
		  if(nReductions >= maxReductions)
		    {
		      if (m_verbosity >= 1) pout ()<< " Luca:: Warning, allowing only 1 reduction per cycle" << endl;
		      break;
		    }
                }
      ...
      if (m_verbosity >= 2) pout() << " setting m_dt_new of " << a_level << " = " << m_dt_new[a_level] << endl;
...
void AMR::regrid(int a_base_level)
{
  ...
  if (m_verbosity >= 2)
    {
      pout() << "AMR::Orig regrid(" << a_base_level << ")" << endl;
    }
...
// generate new grid hierarchy
void AMR::regrid(Vector<IntVectSet>& a_tags, int a_base_level)
{
  CH_TIME("AMR::regrid");
  ...
  m_finest_level_old = m_finest_level;
  int top_level = Min(m_finest_level, m_max_level - 1);
  CH_assert(top_level+1 == a_tags.size() );
          ...
          if (m_verbosity >= 4)
            {
              ...
              if (m_verbosity >= 5)
                {
                  pout() << "AMR::regrid: tags[" << level << "]: " << a_tags[level]
                         << endl;
                }
            }
        }
...
void AMR::initialTime(Real a_initialTime)
{
  ...
  CH_assert(m_isDefined);
  // CH_assert(!m_isSetUp); //lucacommentedout to allow for reset of time
...
//-----------------------------------------------------------------------
Real AMR::getUpdatedTime() const
{
  return (m_cur_time+m_old_dt_base);
}
//-----------------------------------------------------------------------
...
void AMR::initialGrid()
{
  ...
  if (m_use_meshrefine)
    {
      for (int top_level = 0;   top_level < Min(m_finest_level+1, m_max_level);       ++top_level)
        {
          if (m_verbosity >= 2)
            {
              pout() << "AMR::initialGrid: top level = " << top_level << "Finest Level: " << m_finest_level << endl;
            }
	  ...
	  if (m_verbosity >= 5)
	    {
	      pout() << top_level << " New finest Level " << m_finest_level << endl;
	    }
...
#ifdef CH_USE_HDF5
  string iter_str = checkPointName();
...
void AMR::oneStep()
{
  s_step = m_cur_step; // not sure about this value
  m_old_dt_base = m_dt_base;
  Real initial_time = m_cur_time;
  for (int level = 0; level <= m_max_level; ++level)
    {
      m_amrlevels[level]->time(m_cur_time);
    }
  ...
  // updates m_dt_base for next step
  assignDt();
...
//-----------------------------------------------------------------------
Real AMR::getOldDt() const
{
  return m_old_dt_base;
}
//-----------------------------------------------------------------------
Real AMR::getNewDt(const int& a_ilev) const
{
  return m_dt_new[a_ilev];
}
//-----------------------------------------------------------------------
void AMR::setNewDt(const Real& a_dt )
{
  m_dt_base   = a_dt;
  m_dt_new[0] = a_dt;
  m_dt_cur[0] = a_dt;
  m_amrlevels[0]->dt(m_dt_new[0]);
  for (int level = 1; level <= m_max_level; ++level)
      ...
      for (int ilev = 0; ilev < level; ++ilev)
	{
      ...
      m_dt_new[level] = a_dt/ ref_factor;
      m_dt_cur[level] = m_dt_new[level];
      m_amrlevels[level]->dt(m_dt_new[level]);
      if (m_verbosity >= 2) pout() << " setNewDt m_dt_new of " << level << " = " << m_dt_new[level] << endl;
    }
}
bool AMR::ok(const Real a_max_time, const int a_max_step) const
{
  return (m_cur_step < a_max_step) &&
    (a_max_time - m_cur_time > m_time_eps*m_dt_base);
}
void AMR::add()
{
  ++ m_cur_step;
  m_cur_time += m_old_dt_base;   
}
int AMR::getStep() const
{
  return m_cur_step;   
}
void AMR::setStep(int a_steap)
{
  m_cur_step = a_steap;   
} 
int AMR::getFinestLevel() const
{
  return m_finest_level;   
}

bool AMR::dumpCheckPoint() const
{
  return ( (m_checkpoint_interval > 0)      &&
	   (m_lastcheck_step != m_cur_step) &&
	   (m_restart_step != m_cur_step)   &&
	   (m_cur_step % m_checkpoint_interval == 0) );   
}
std::string AMR::checkPointName() const
{
