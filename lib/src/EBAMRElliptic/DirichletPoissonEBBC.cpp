...
Real DirichletPoissonEBBC::getEBValue(const VolIndex&               a_vof,
				      const DataIndex&              a_dit,
				      const RealVect&               a_probLo,
				      const EBISBox&                a_curEBISBox,
				      const Real&                   a_time)
{
  CH_TIME("DirichletPoissonEBBC::getEBValue");
  //  CH_assert(a_phi.nComp() == 1);
  ...
  point *= 0.5;
  point += iv;
  point += a_curEBISBox.bndryCentroid(a_vof);
  point *= m_dx;
  point += a_probLo;
  ...
  if (m_dataBased)
    ...
    {
      RealVect normal = a_curEBISBox.normal(a_vof);
      value = m_func->value(point, normal, a_time,comp);
    }
  else
    {
      if (m_onlyHomogeneous)
