...
Real EBAMRPoissonOp::s_slowSafetyFactor = 0.5;
  ...
  for (int idir = 0; idir < SpaceDim; idir++)
    {
      ...
      m_cacheInhomDomBCLo[idir].define( m_eblg.getDBL());
      m_cacheInhomDomBCHi[idir].define( m_eblg.getDBL());
    }
      ...
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          ...
          m_cacheInhomDomBCLo[idir][dit()].define(curEBISBox,curBox,1);
          m_cacheInhomDomBCHi[idir][dit()].define(curEBISBox,curBox,1);
         }
          ...
          for (vofitlo.reset(); vofitlo.ok(); ++vofitlo)
            {
              ...
              m_cacheInhomDomBCLo[idir][dit()](vof, 0) = flux;
            }//vofitlo
          ...
          for (vofithi.reset(); vofithi.ok(); ++vofithi)
            {
              ...
              m_cacheInhomDomBCHi[idir][dit()](vof, 0) = flux;
            }//vofithi
        }//idir
...
{
  //checkLevel(a_phi, string("phiR "));
  //checkLevel(a_rhs, string("rhsR "));
      ...
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          ...
          m_opEBStencilInhomDomLo[idir][a_dit]->apply(curOpPhiEBCellFAB,
                                                      m_cacheInhomDomBCLo[idir][a_dit()],
                                                      one,
                                                      alpha,
                                                      1.,
                                                      true);
          ...
          m_opEBStencilInhomDomHi[idir][a_dit]->apply(curOpPhiEBCellFAB,
                                                      m_cacheInhomDomBCHi[idir][a_dit()],
                                                      one,
                                                      alpha,
                                                      1.,
                                                      true);
...
setEBBCSource(const Vector<LevelData<BaseIVFAB<Real> >* > a_rhos, const Vector<ProblemDomain>& a_domains)
{
  ...
  if (!found)
    {
      MayDay::Error("EBAMRPoissonOp::setEBBCSource cant find level");
    }
              ...
              const IntVect&              a_color)
{
  ...
  //LM change very slow relaxation
  Real safety = s_slowSafetyFactor;
...
void
EBAMRPoissonOp::
printMaxMin(const LevelData<EBCellFAB>&  a_data, const string& a_msg) const
{ 
  
  Real maxElec, minElec; 
  EBLevelDataOps::getMaxMin(maxElec, minElec, a_data,0);
  pout() << a_msg << ", " << maxElec << ", " <<  minElec << endl;
}
