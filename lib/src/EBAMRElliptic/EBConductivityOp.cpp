               ...
               const DataIndex& a_dit)
{
  ...
  FaceStencil interpSten = EBArith::getInterpStencil(a_face, (*m_eblg.getCFIVS())[a_dit], m_eblg.getEBISL()[a_dit], m_eblg.getDomain());
...
void EBConductivityOp::
setEBBCSource(const LevelData<BaseIVFAB<Real> >&     a_rhos)
{
  m_ebBC->setEBBCSource(a_rhos);
}
//-----------------------------------------------------------------------
void
EBConductivityOp::
      ...
      for (int idir = 0; idir < SpaceDim; idir++)
        {
          ...
          for (SideIterator sit; sit.ok(); ++sit)
            {
              Vector<FaceIndex>*  faceit= nullptr;
              Vector<VoFStencil>* stencil= nullptr;
