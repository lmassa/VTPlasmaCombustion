                              ...
                              const Real&                   a_factor)
{
  ...
  if ((m_ebType == 1) || (m_ebType == 10))
      ...
      for (DataIterator dit = dbl.dataIterator(); dit.ok(); ++dit)
	{
	  ...
	  IntVectSet notRegularOld = curEBISBox.getIrregIVS  (oldBox);
	  ...
	  m_rhos[dit()].define(notRegularOld,curEBGraph,nComps);
      ...
      if( fabs(m_epsHi/m_epsLo -1.0) >  1e-6) defineCustom(a_cfivs, a_factor);
      m_isDefined = true;
...
void MixedPoissonEBBC::defineCustom(const LayoutData<IntVectSet>& a_cfivs, const Real& a_factor)
{
  m_finest = (m_dx[1] == m_dymin[1]); 
  ...
  if (m_ebType == 10)
      ...
      for (DataIterator dit = dbl.dataIterator(); dit.ok(); ++dit)
	{
	  ...
	  for (VoFIterator vofit(notRegularOld,curEBGraph); vofit.ok(); ++vofit)
	    { 
	      const VolIndex& vof = vofit();
		  ...
		  if (curBox.contains(ivLo))
		      ...
		      else
			{
			  ...
			  if (curBox.contains(ivHi))
			    ...
			    {
			      pout() << "MixedPoissonEBBC::defineCustom, Misplaced Boxes " << ivLo << ivHi << endl;
			      MayDay::Error("EB BC defineCustom Misplaced Boxes");
			    }
			}			
		    }
		  else
		    {
		      pout() << "MixedPoissonEBBC::defineCustom, curBox does not contain(ivLo) " << ivLo << endl;
		      MayDay::Error("EB BC defineCustom curBox does not contain(ivLo)");
		    }
...
/******************************************/
void MixedPoissonEBBC::zeroEBBCSource()
{

  const DisjointBoxLayout& dbl = m_layout.getDisjointLayout();
  for (DataIterator dit = dbl.dataIterator(); dit.ok(); ++dit) m_rhos[dit()].setVal(0e0);
}
